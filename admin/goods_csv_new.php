...
if(isset($_GET['action']))
{
	switch($_GET['action'])
	{
		case 'import':
			...
			$type = $_POST['type'];        
			if (!$type) errorAlert('Выберите тип импорта');
			if ($type != "1" && $type != "2") errorAlert('Неверный тип импорта: '.$type);
			...
			while($row = fgetcsv($file,100000,';')) 
			{
				if($i++>=$start)
				{
					...
					if(!$text) { fwrite($log,"{$kod};Нет описания товара в файле\n"); $errors++; continue; }
										
					$good = getRow("SELECT id,text{$type} FROM {$prx}goods WHERE id IN ({$ids_good}) AND (kod='{$kod}' OR kod2='{$kod}')");
					if(!$good) { fwrite($log,"{$kod};Нет кода товара указанного бренда на сайте\n"); $errors++; continue;	}

					$text_field = "text{$type}";
					if($good[$text_field]) { fwrite($log,"{$kod};Текст был изменен\n"); $errors++; }
			...
			$r = mysql_query("SELECT kod,text{$type} FROM {$prx}goods WHERE length(text{$type}) < 20");
...
<table class="content" width="450">
  ...
  <tr>
            <td>Тип импорта</td>
            <td>
                <input type="radio" name="type" value="1" checked> в краткое описание</input><br>
                <input type="radio" name="type" value="2"> в полное описание</input>
            </td>
  </tr>
  <tr>
    <td>Файл</td>
